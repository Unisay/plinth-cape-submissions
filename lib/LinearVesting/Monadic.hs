...
respond to inline-unconditional-growth here (measured on the HTLC port:
every swept cell produced a byte-identical artifact).
-}
...
-}
module LinearVesting.Monadic (
  linearVestingValidatorCode,
  linearVestingValidator,
) where

import LinearVesting (VestingDatum)
...
validatePartialUnlock txInfo scriptInfo = V.do
  ...
  -- spine walk beats a beneficiary/firstUnlockAfter + rest split
  -- (measured on the 1.65 line under happy-path-only scoring).
  ( beneficiary
    , asset
    , totalQty
    , periodStart
