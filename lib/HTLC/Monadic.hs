...
{- No inliner-budget pragma on this branch: the 1.45.0.0 plugin does not
respond to inline-unconditional-growth here — swept default/24/32/52/200
and every cell produced a byte-identical artifact.
-}
...
free — so the decode plan serves the happy path alone. The measured tradeoffs:
  ...
  * one 'fields' region extracts a @Constr@'s demanded fields in ONE spine
    ...
    demanded field. Under happy-path-only scoring the per-path 'HTLCDatum'
    regions (claim: recipient/secretHash/timeout, refund: payer/timeout)
    replaced the lazy selectors for −3 547 total_fee_lovelace on this line;
    the earlier lazy-selector advantage existed only because the old
    aggregation also summed the abort paths, which skip fields their failing
    guard never reaches.
